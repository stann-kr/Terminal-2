...
export default function DirectoryLink({ href, label, description, index, accent = 'primary', external = false }: DirectoryLinkProps) {
  ...
  const inner = (
    <motion.div
      ...
      className={`flex items-start gap-4 py-3 px-4 border-b border-terminal-accent-primary/10 bg-transparent transition-[border-color,background-color,box-shadow] duration-200 ${v.hover}`}
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-3 mb-0.5">
          <span
            className={`text-small font-bold tracking-widest uppercase transition-[color,filter] duration-150 ${hovered ? `${v.text} ${v.glow}` : 'text-terminal-primary'}`}
          >
